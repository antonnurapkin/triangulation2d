...
class Triangle final {
   public:
    ...
    // for points
    std::array<int, 3> get_points_indexes() const {
        return indexes_;
    }

    int get_point_index(int index) const {
        return indexes_[index];
    }

    void set_point_index(int point_index, int place_index) {
        indexes_[place_index] = point_index;
    }
    ...
    // for adjacents
    bool is_adjacents_exist() const {
        return true ? adjacentTriangles_.size() > 0 : false;
    }

    std::shared_ptr<Triangle> get_adjacent(int index) const {
        return adjacentTriangles_[index];
    }
    ...
    std::vector<std::shared_ptr<Triangle>> get_all_adjacents() {
        return adjacentTriangles_;
    }

    void clear_adjacents() {
        adjacentTriangles_.clear();
    }

    void print_points_indexes() const {
        std::cout << std::to_string(indexes_[0]) + " " + std::to_string(indexes_[1]) + " " + std::to_string(indexes_[2]) << std::endl;
    }
