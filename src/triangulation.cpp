...
std::shared_ptr<Triangle> triangulation::find_triangle(const Point& point, const std::shared_ptr<Triangle>& cur_triangle,
                                                       const std::vector<Point>& points) {
    Point center = utils::get_centroid(cur_triangle, points);

    std::vector<double> N1 = utils::cross_product_with_normal(
        utils::vector_by_points(points[cur_triangle->get_points_indexes()[0]], points[cur_triangle->get_points_indexes()[1]]));

    std::vector<double> N2 = utils::cross_product_with_normal(
        utils::vector_by_points(points[cur_triangle->get_points_indexes()[1]], points[cur_triangle->get_points_indexes()[2]]));

    std::vector<double> N3 = utils::cross_product_with_normal(
        utils::vector_by_points(points[cur_triangle->get_points_indexes()[2]], points[cur_triangle->get_points_indexes()[0]]));
...
std::shared_ptr<Triangle> triangulation::add_new_triangles(int index, std::shared_ptr<Triangle>& parent_triangle,
                                                           std::vector<std::shared_ptr<Triangle>>& triangles, const std::vector<Point>& points) {
...
void triangulation::add_external_adjacent(std::shared_ptr<Triangle>& new_tri, std::shared_ptr<Triangle>& parent_tri,
                                          const std::vector<Point>& points) {
...
void triangulation::update_adjacent_neighbors(const std::shared_ptr<Triangle>& new_triangle, std::shared_ptr<Triangle>& parent_triangle,
                                              const Point& point_1, const Point& point_2, const std::vector<Point>& points) {
...
bool triangulation::check_delauney_condition(const std::shared_ptr<Triangle>& tri, const std::shared_ptr<Triangle>& adj,
                                             const std::vector<Point>& points) {
    // Находим общее ребро
    ...
    auto in_circle = [](const Point& A, const Point& B, const Point& C, const Point& D) -> bool {
        ...
        double orient = (B.get_x() - A.get_x()) * (C.get_y() - A.get_y()) - (B.get_y() - A.get_y()) * (C.get_x() - A.get_x());
...
void triangulation::set_new_adjacents(std::shared_ptr<Triangle>& triangle, std::shared_ptr<Triangle>& other_triangle,
                                      std::vector<std::shared_ptr<Triangle>>& adjacents, const std::vector<Point>& points) {
...
triangulation::trianglesArrayLikeDataType triangulation::convert_to_array_like(const std::vector<std::shared_ptr<Triangle>>& triangles,
                                                                               const std::vector<Point>& points,
